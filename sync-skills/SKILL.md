---
name: sync-skills
description: "Pull/sync custom skills from the claude-skills GitHub repository. Use when the user wants to: sync skills, pull skills, download skills from GitHub, update skills from another computer, or any mention of syncing/pulling claude skills."
---

# Sync Custom Skills from GitHub

Pull the latest custom skills from the `claude-skills` GitHub repository into `~/.claude/skills/`.
...
3. Check if there are incoming changes: `git -C ~/.claude/skills diff HEAD..origin/main --stat`
   - If no changes, report "Already up to date. No new skills to sync." and stop
...
1. List the custom skill directories that now exist (non-symlink, non-.git dirs):
   ```
   find ~/.claude/skills -maxdepth 1 -mindepth 1 -type d ! -type l -not -name '.git' -exec basename {} \;
   ```
2. Tell the user:
